...
var prealert_description = $('#prealert_description').val();

var packagesItems = [
    {
        qty: 1,
        description: prealert_description,
        length: 0,
        width: 0,
        height: 0,
        weight: 0,
...
$("#invoice_form").on('submit', function (event) {
    ...
    var prealert_id = $('#prealert_id').val();
    ...
    if (prealert_id) { data.append('prealert_id', prealert_id) }
    ...
    $.ajax({
        type: "POST",
        url: "ajax/customers_packages/add_customers_packages_from_prealert_ajax.php",
        data: data,
        contentType: false,
        dataType: 'json',
