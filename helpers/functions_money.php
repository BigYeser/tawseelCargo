...
class License2l6aspi3ekdz14bfnxwtugjycm05hqcdpcoddingproV7
{
    ...
    private function calledxs5wohkrfju1_api($method, $url, $data = NULL)
    {
        $curl = curl_init();
        switch ($method) {
            case "POST":
    ...
    {
        $get_data = $this->calledxs5wohkrfju1_api("POST", $this->api_url . "api/check_connection_ext");
    ...
    public function get_latest_version()
    {
        ...
        $get_data = $this->calledxs5wohkrfju1_api("POST", $this->api_url . "api/latest_version", json_encode($data_array));
        $response = json_decode($get_data, true);
        return $response;
    }
    public function activate_license($license, $client, $create_lic = true)
    {
        ...
        $get_data = $this->calledxs5wohkrfju1_api("POST", $this->api_url . "api/activate_license", json_encode($data_array));
        $response = json_decode($get_data, true);
    ...
    public function verify_license($time_based_check = false, $license = false, $client = false)
    {
        ...
        if ($time_based_check && 0 < $this->verification_period) {
            ...
            if (strtotime($_SESSION["eae6a6b0ab85366"]) <= strtotime($today)) {
                $get_data = $this->calledxs5wohkrfju1_api("POST", $this->api_url . "api/verify_license", json_encode($data_array));
    ...
    public function deactivate_license($license = false, $client = false)
    {
        ...
        $get_data = $this->calledxs5wohkrfju1_api("POST", $this->api_url . "api/deactivate_license", json_encode($data_array));
        $response = json_decode($get_data, true);
        if ($response["status"]) {
            @chmod($this->license_file, 511);
            if (is_writeable($this->license_file)) {
    ...
    public function check_update()
    {
        ...
        $get_data = $this->calledxs5wohkrfju1_api("POST", $this->api_url . "api/check_update", json_encode($data_array));
        $response = json_decode($get_data, true);
        return $response;
    }
